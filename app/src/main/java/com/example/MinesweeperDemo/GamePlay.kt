...
class GamePlay: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Handler(Looper.getMainLooper()).postDelayed({
            ...
            val intent= Intent(this, results::class.java).apply{
                putExtra("result","Time is over, You Lost\nTry Again")
            }
            startActivity(intent)
        }, 180000)

        val actionbar = supportActionBar
        actionbar!!.title = "Play Game"
    ...
    private fun setUpBoard(row: Int, col: Int, mine: Int) {
        ...
        mineFlag.setOnClickListener{
            if(choice==1) {
                mineFlag.setImageResource(R.drawable.flag_icon)
                choice=2
            }else{
        ...
        cellBoard.forEach { row ->
            row.forEach {
                if(it.isRevealed)
                    ...
                    it.setBackgroundResource(R.drawable.flag_icon)
